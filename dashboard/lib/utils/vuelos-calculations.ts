...
export function calcularFechaLimiteAnulacion(fecha_vuelo: string, dias_antes: number = 7): string {
  const fecha = new Date(fecha_vuelo);
  fecha.setDate(fecha.getDate() - dias_antes);
  return fecha.toISOString().split('T')[0];
}

export function formatearMontoWhatsApp(monto: number): string {
...
export function validarLocalizador(localizador: string): boolean {
  return /^[A-Z0-9]{5,8}$/i.test(localizador);
}

export function validarCodigoIATA(codigo: string): boolean {
  return /^[A-Z]{2}$/i.test(codigo);
}

export function calcularTotalPasajeros(num_adultos: number, num_ninos: number, num_infantes: number): number {
