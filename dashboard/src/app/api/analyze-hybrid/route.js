  ...
  try {
    ...
    for (let i = 0; i < conversations.length; i++) {
      ...
      try {
        ...
        const messagesResponse = await fetch(
          `${process.env.NEXT_PUBLIC_APP_URL}/api/get-messages`,
          {
            method: "POST",
            headers: {
        ...
        if (!messagesResponse.ok) {
          console.warn(
            `No se pudieron obtener mensajes para conversación ${conversation.id}`,
          );
          continue;
        }
        ...
        const analysis = await analyzeCompletePerformance(messages, {
          chat_id: conversation.id,
          contact_name:
            conversation.contact_name || conversation.name || "Cliente",
        ...
        if (analysis.success && analysis.evaluation) {
          ...
          if (scoreTotal < 40) {
            results.combined_summary.conversaciones_criticas.push({
              ...
              score: scoreTotal,
              resultado:
                evaluation.resultado_comercial?.tipo || "SIN_CLASIFICAR",
              area_critica:
                analysis.analysis_summary?.area_critica || "GENERAL",
            });
          }
          ...
          if (analysis.recommendations) {
            ...
            if (
              evaluation.resultado_comercial?.prioridad === "ALTA" ||
          ...
          console.error(
            `Error en análisis de conversación ${conversation.id}:`,
            analysis.error,
          );
        }
    ...
    if (results.total_processed > 0) {
      ...
      results.sales_summary.tasa_conversion = (
        (results.sales_summary.ventas_confirmadas / results.total_processed) *
        100
      ).toFixed(1);
      ...
      results.sales_summary.valor_promedio =
        results.sales_summary.ventas_confirmadas > 0
          ? (
              results.sales_summary.valor_total /
              results.sales_summary.ventas_confirmadas
            ).toFixed(0)
          : 0;
      ...
      const totalScores = Object.values(results.evaluations).reduce(
        (sum, evaluation) => sum + parseFloat(evaluation.percentage_total || 0),
        0,
      );

      results.combined_summary.score_promedio_total = (
        totalScores / results.total_processed
      ).toFixed(1);

      // Score promedio de proceso
      const processScores = Object.values(results.evaluations).reduce(
        (sum, evaluation) =>
          sum + parseFloat(evaluation.percentage_proceso || 0),
        0,
      );

      results.process_summary.score_promedio_proceso = (
        processScores / results.total_processed
      ).toFixed(1);
      ...
      Object.values(results.evaluations).forEach((evaluation) => {
        if (!evaluation.venta_confirmada)
          parametrosFallos.venta_confirmada =
            (parametrosFallos.venta_confirmada || 0) + 1;
        if (!evaluation.lead_caliente)
          parametrosFallos.lead_caliente =
            (parametrosFallos.lead_caliente || 0) + 1;
        if (!evaluation.cotizacion_enviada)
          parametrosFallos.cotizacion_enviada =
            (parametrosFallos.cotizacion_enviada || 0) + 1;
        if (!evaluation.seguimiento_efectivo)
          parametrosFallos.seguimiento_efectivo =
            (parametrosFallos.seguimiento_efectivo || 0) + 1;
      });

      results.process_summary.parametros_criticos = Object.entries(
        parametrosFallos,
      )
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
      ...
      results.recommendations.mejoras_prioritarias = [
        ...new Set(results.recommendations.mejoras_prioritarias),
      ].slice(0, 5);
    }
