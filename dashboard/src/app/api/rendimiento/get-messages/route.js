...
export async function POST(request) {
  // Create Supabase client in runtime (not at module level)
  ...
  try {
    ...
    // Detectar si chatId es un UUID o un WhatsApp ID
    const isWhatsAppId = chatId && (chatId.includes('@') || /^\d+$/.test(chatId));
    const queryField = isWhatsAppId ? 'chat_id' : 'id';
    const queryValue = isWhatsAppId ? (chatId.includes('@') ? chatId : `${chatId}@c.us`) : chatId;

    console.log(`   🔍 Tipo de consulta: ${queryField} = ${queryValue}`);
    ...
    console.log(`   ✓ Chat UUID: ${chatData.id}`);
    console.log(`   ✓ WhatsApp ID: ${chatData.chat_id}`);

    // IMPORTANTE: La tabla messages.chat_id es UUID que referencia chats.id
    const chatUuidForMessages = chatData.id;
    
    console.log(`   🔍 Consultando mensajes con chat_id (UUID): ${chatUuidForMessages}`);

    // Consultar mensajes usando el UUID del chat
    const { data, error, count } = await supabase
      .from('messages')
      .select('from_me, body, content, timestamp, type', { count: 'exact' })
      .eq('chat_id', chatUuidForMessages)
