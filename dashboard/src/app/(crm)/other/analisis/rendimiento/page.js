...
import { getOtherDashboardStats } from "@/lib/supabaseRendimiento";

export default function OtherRendimientoPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const lastViewed = localStorage.getItem('other:lastViewedAdvisor');
      if (lastViewed) {
  ...
  const loadAdvisorsData = async () => {
    try {
      setLoading(true);
      const stats = await getOtherDashboardStats();
      ...
      const advisorsData = stats.map((stat) => {
        ...
        const total = latestAnalysis.total_conversations_analyzed || 1;
        const metrics = {
          tiempo_contacto: (latestAnalysis.tiempo_contacto_count || 0) > total * 0.7,
          tiempo_respuesta: (latestAnalysis.tiempo_respuesta_count || 0) > total * 0.7,
          tiempo_cotizacion: (latestAnalysis.tiempo_cotizacion_count || 0) > total * 0.7,
          cierre_intencion: (latestAnalysis.cierre_intencion_count || 0) > total * 0.7,
          ofrecio_scalapay: (latestAnalysis.ofrecio_scalapay_count || 0) > total * 0.7,
          mas_dos_opciones: (latestAnalysis.mas_dos_opciones_count || 0) > total * 0.7,
          seguimiento_intencion: (latestAnalysis.seguimiento_intencion_count || 0) > total * 0.7,
        };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          {/* Breadcrumb */}
          <Breadcrumb items={[
            { label: "Other", href: "/other/conversaciones" },
            { label: "Análisis", href: "/other/analisis/rendimiento" },
            { label: "Rendimiento", href: "/other/analisis/rendimiento" }
          ]} />
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
                  ...
                  Control de Rendimiento Other
                </h1>
                <p className="text-gray-600 mt-1">
                  Seguimiento diario del desempeño de los bots Other
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {displayedAdvisors.map((advisor) => (
              <AdvisorPerformanceCard
                ...
                onClick={() => {
                  localStorage.setItem('other:lastViewedAdvisor', advisor.id);
                }}
              />
            ))}
          </div>
