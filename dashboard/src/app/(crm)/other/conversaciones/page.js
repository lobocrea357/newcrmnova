...
import { useOtherBots } from '@/hooks/useBots'
...
function OtherDashboardContent() {
  const { user, session } = useAuth();
  const { bots, loading: botsLoading } = useOtherBots();
  const [conversations, setConversations] = useState({});
  const [conversationsPagination, setConversationsPagination] = useLocalStorage('other:conversaciones:botPages', {});
  ...
  const [lastChatId, setLastChatId] = useLocalStorage('other:conversaciones:lastChatId', null);
  ...
  const [lastSearchQuery, setLastSearchQuery] = useLocalStorage('other:conversaciones:lastSearchQuery', '');
  ...
  const handleBotSelect = async (botId) => {
    ...
    router.push(`/other/conversaciones?${params.toString()}`);
  };
  ...
  const handleConversationClick = (botId, chatId) => {
    ...
    router.push(`/other/conversaciones/chat/${chatId}?botId=${botId}`);
  };
  ...
  const handleGlobalSearchResultClick = (chat) => {
    ...
    router.push(
      `/other/conversaciones/chat/${chatIdStr}?botId=${botIdStr}&fromSearch=true`,
    );
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando dashboard other...</p>
        </div>
      </div>
    );
  }
...
export default function OtherConversationsPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
