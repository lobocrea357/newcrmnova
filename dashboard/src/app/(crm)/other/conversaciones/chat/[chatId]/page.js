...
function OtherChatPageContent() {
  ...
  const [globalSearchQuery, setGlobalSearchQuery] = useLocalStorage('other:conversaciones:chatSearchQuery', '')
  ...
  const handleClose = () => {
    ...
    if (botId) {
      router.push(`/other/conversaciones?botId=${botId}&chatId=${chatId}`);
    } else {
  ...
  const handleSearchResultClick = (chat) => {
    ...
    router.push(
      `/other/conversaciones/chat/${newChatId}?botId=${botId}&fromSearch=true`,
    );
  };
